...
import { Search, Filter, Infinity as InfinityIcon, Clock, Star, ChevronDown, LayoutGrid, List, SlidersHorizontal } from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-8">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase flex items-center drop-shadow-md">
                        Catálogo
                        <span className="ml-3 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[10px] font-black px-2.5 py-1 rounded-md uppercase tracking-widest shadow-md">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                    <div className="relative flex-1 group">
                        ...
                        <input 
                            ...
                            className="w-full pl-11 pr-4 py-4 rounded-2xl border border-white/5 bg-[#0a0f1c]/80 backdrop-blur-md text-white outline-none focus:border-cyan-500/50 transition-all font-bold text-sm placeholder:text-gray-500 shadow-inner" 
                            placeholder="Buscar títulos, autores..." 
                        />
                    </div>

                    <div className="flex items-center gap-3">
                        <button onClick={() => setShowFilters(!showFilters)} className={`flex-1 sm:flex-none backdrop-blur-md border px-5 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-xs uppercase tracking-widest transition-all ${showFilters ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.2)]' : 'bg-[#0a0f1c]/80 border-white/5 text-gray-300 hover:bg-white/5 hover:text-white'}`}>
                            <SlidersHorizontal className="w-4 h-4" /> Filtros
                        </button>
                        
                        {/* VIEW MODE TOGGLE */}
                        <div className="flex items-center bg-[#0a0f1c]/80 backdrop-blur-md border border-white/5 rounded-2xl p-1 shadow-sm h-full">
                            <button onClick={() => setViewMode('grid')} className={`p-2.5 rounded-xl transition-colors ${viewMode === 'grid' ? 'bg-cyan-500 text-black shadow-md' : 'text-gray-500 hover:text-white'}`}>
                                <LayoutGrid className="w-4 h-4" />
                            </button>
                            <button onClick={() => setViewMode('list')} className={`p-2.5 rounded-xl transition-colors ${viewMode === 'list' ? 'bg-cyan-500 text-black shadow-md' : 'text-gray-500 hover:text-white'}`}>
                ...
                {showFilters && (
                    <div className="bg-[#0a0f1c]/90 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-4 sm:p-5 mb-6 shadow-[0_10px_30px_rgba(0,0,0,0.5)] animate-in slide-in-from-top-2">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">Tipo</label>
                                <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="bg-[#050505] border border-white/10 text-white text-xs font-bold rounded-xl px-3 py-3 outline-none focus:border-cyan-500 appearance-none">
                                    {typeOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                </select>
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">Gênero</label>
                                <select value={selectedGenre} onChange={(e) => setSelectedGenre(e.target.value)} className="bg-[#050505] border border-white/10 text-white text-xs font-bold rounded-xl px-3 py-3 outline-none focus:border-cyan-500 appearance-none">
                                    {genreOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                </select>
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">Status</label>
                                <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="bg-[#050505] border border-white/10 text-white text-xs font-bold rounded-xl px-3 py-3 outline-none focus:border-cyan-500 appearance-none">
                                    {statusOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                </select>
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <label className="text-[9px] font-black text-cyan-500 uppercase tracking-widest">Ordenar Por</label>
                                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-[#050505] border border-white/10 text-white text-xs font-bold rounded-xl px-3 py-3 outline-none focus:border-cyan-500 appearance-none">
                ...
                {/* PÍLULAS DE ACESSO RÁPIDO (Atalhos para Tipos) */}
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-6 mb-2">
                    {typeOptions.map(opt => (
                        <button 
                            key={opt} 
                            onClick={() => setSelectedType(opt)} 
                            className={`px-6 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest whitespace-nowrap transition-all duration-200 border ${selectedType === opt ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-black border-transparent shadow-[0_0_15px_rgba(34,211,238,0.4)]' : 'bg-[#0a0f1c]/80 text-gray-400 hover:text-white border-white/5 hover:border-cyan-500/30'}`}
                        >
                            {opt}
                        </button>
                    ))}
