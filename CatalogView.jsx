...
import { Infinity as InfinityIcon, Star, Clock, ChevronDown } from 'lucide-react';
import { timeAgo } from './helpers';

export function CatalogView({ mangas, onNavigate, dataSaver, catalogState, setCatalogState }) {
    
    // Pegando o estado atual salvo (para manter onde o usuário estava)
    const filterType = catalogState?.filterType || "Todos";
    const visibleCount = catalogState?.visibleCount || 24;

    // Lista expandida de gêneros e tipos
    const filterOptions = [
        'Todos', 'Ação', 'Artes Marciais', 'Aventura', 'Comédia', 'Drama', 'Esportes', 
        'Fantasia', 'Ficção Científica', 'Isekai', 'Magia', 'Mistério', 'Romance', 
        'Seinen', 'Shoujo', 'Shounen', 'Slice of Life', 'Terror', 'Manhwa', 'Mangá', 'Manhua'
    ];
    ...
    // Lógica de filtro (Apenas Categoria/Gênero)
        ...
        <div className="pb-24 animate-in fade-in duration-500 bg-[#050505] min-h-screen relative font-sans text-white">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-8">
                ...
                <div className="flex items-center gap-3 mb-4">
                    <div className="bg-[#0a0a0a] p-2.5 rounded-xl border border-white/5 shadow-lg">
                    ...
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase flex items-center drop-shadow-md">
                        Catálogo
                        <span className="ml-3 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-widest shadow-md">
                            {filteredMangas.length} Obras
                        </span>
                    </h1>
                </div>

                {/* FILTROS FLUTUANTES (Sticky no topo) */}
                <div className="sticky top-[64px] z-40 bg-[#050505]/95 backdrop-blur-md border-b border-white/5 py-4 mb-8 -mx-4 px-4 sm:mx-0 sm:px-0">
                    <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                        {filterOptions.map(opt => (
                            <button 
                                key={opt} 
                                onClick={() => handleTypeChange(opt)} 
                                className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all duration-200 border ${filterType === opt ? 'bg-cyan-500 text-black border-cyan-500 shadow-[0_0_15px_rgba(34,211,238,0.3)]' : 'bg-[#0a0a0a] text-gray-400 hover:text-white hover:bg-white/5 border-white/5 hover:border-cyan-500/30'}`}
                            >
                                {opt}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-5">
                    {currentItems.length > 0 ? currentItems.map(manga => (
                        ...
                        <div className="col-span-full py-24 flex flex-col items-center justify-center bg-[#0a0a0a]/50 rounded-3xl border border-white/5 border-dashed">
                            ...
                            <p className="text-gray-400 font-black text-xs uppercase tracking-widest text-center px-4">O Vazio não revelou nenhum resultado para este filtro.</p>
                        </div>
                    )}
                </div>
