    ...
    <div className={`animate-in fade-in duration-700 w-full pb-24 font-sans min-h-screen text-gray-200 ${eq.tema_perfil ? eq.tema_perfil.cssClass : 'bg-transparent'}`}>
      ...
      {confirmAction && (
          <div className="fixed inset-0 z-[3000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
              <div className="bg-[#0a0a0f] border border-white/10 p-8 rounded-3xl max-w-sm w-full text-center shadow-2xl">
                  ...
                  <div className="flex gap-3">
                      ...
                      <button onClick={executeConfirmAction} className="flex-1 bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 font-bold py-3.5 rounded-xl transition-colors hover:bg-cyan-500 hover:text-white text-xs">Confirmar</button>
                  </div>
              </div>
          </div>
      )}
      ...
      {/* ========================================================= */}
      {/* CAPA (COVER) - Ocupando o topo com corte seco e altura fixa */}
      {/* ========================================================= */}
      <div className="w-full h-[220px] md:h-[280px] bg-[#050508] relative group">
        ...
        {/* Sombra levíssima no pé da capa apenas para não cortar seco na cor da página */}
        <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#020204]/80 to-transparent" />
      ...
      {/* ========================================================= */}
      {/* PROFILE CARD FLUTUANTE (Glassmorphism real)                 */}
      {/* ========================================================= */}
      <div className="max-w-3xl mx-auto px-4 sm:px-6 relative z-10 -mt-[90px]">
        
        {/* O card translúcido e moderno */}
        <div className="bg-white/[0.04] backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[24px] shadow-[0_8px_32px_rgba(0,0,0,0.4)] flex flex-col items-center text-center relative">
          
          {/* AVATAR: Puxado para cima, "quebrando" a borda do card, estilo app nativo */}
          <div className={`relative w-28 h-28 -mt-[76px] mb-3 flex-shrink-0 group ${(!eq.moldura?.preview && eq.moldura) ? eq.moldura.cssClass : ''}`}>
            <div className={`w-full h-full rounded-full bg-[#0a0a0f] flex items-center justify-center relative z-10 overflow-hidden shadow-2xl ${!eq.moldura ? 'border-[4px] border-[#0a0a0f]' : ''}`}>
            ...
            {isEditing && <button onClick={() => avatarInputRef.current.click()} className="absolute bottom-0 right-0 bg-white text-black p-2.5 rounded-full z-50 border-[3px] border-[#0a0a0f] hover:bg-gray-200 transition-colors shadow-lg"><Camera className="w-4 h-4" /></button>}
          ...
          <div className="w-full">
            ...
            {bio && !isEditing && <p className="text-gray-300 text-xs font-medium mt-3 bg-white/5 inline-block px-4 py-2 rounded-xl border border-white/5">{bio}</p>}
            
            {/* Barra de EXP enxuta e elegante */}
            <div className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 mt-5 relative overflow-hidden shadow-inner">
          ...
          <div className="flex gap-3 w-full justify-center mt-6">
            <button onClick={() => setIsEditing(!isEditing)} className="bg-white/10 border border-white/10 text-white px-6 py-2.5 rounded-xl text-[11px] font-bold flex items-center justify-center gap-2 hover:bg-white/20 transition-all backdrop-blur-md shadow-sm"><Edit3 className="w-4 h-4" /> {isEditing ? 'Cancelar' : 'Editar Perfil'}</button>
            <button onClick={onLogout} className="bg-red-500/10 text-red-400 p-2.5 rounded-xl hover:bg-red-600 hover:text-white transition-all border border-red-500/20 backdrop-blur-md"><LogOut className="w-5 h-5" /></button>
          </div>
        </div>
        
        {isEditing && (
          <form onSubmit={handleSave} className="bg-white/[0.04] backdrop-blur-xl border border-white/10 rounded-[24px] p-6 md:p-8 animate-in fade-in shadow-[0_8px_32px_rgba(0,0,0,0.4)] mt-6">
            <div className="space-y-5">
              <div>
                 <label className="block text-[11px] font-bold text-gray-400 mb-2 uppercase tracking-widest">Nome de Exibição</label>
                 <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-black/30 border border-white/5 rounded-xl px-4 py-3.5 text-white text-sm font-medium outline-none focus:border-cyan-500 transition-colors shadow-inner"/>
              </div>
              <div>
                 <label className="block text-[11px] font-bold text-gray-400 mb-2 uppercase tracking-widest">Biografia</label>
                 <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} className="w-full bg-black/30 border border-white/5 rounded-xl px-4 py-3.5 text-white text-sm font-medium resize-none outline-none focus:border-cyan-500 transition-colors shadow-inner"></textarea>
              </div>
            </div>
            <button type="submit" disabled={loading} className="mt-6 bg-white text-black text-[11px] font-black px-8 py-3.5 rounded-xl w-full flex justify-center hover:bg-gray-200 transition-all uppercase tracking-widest shadow-lg">{loading ? <Loader2 className="w-4 h-4 animate-spin"/> : 'Salvar Alterações'}</button>
          </form>
        )}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 mt-8">
        <div className="flex gap-3 mb-8 overflow-x-auto no-scrollbar pb-2 snap-x">
          {['Estatisticas', 'Historico', 'Biblioteca', 'Configuracoes'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`snap-start px-5 py-2.5 rounded-xl font-bold transition-all whitespace-nowrap text-[10px] sm:text-[11px] uppercase tracking-wider flex items-center gap-2 ${activeTab === tab ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30' : 'bg-white/[0.04] text-gray-400 border border-white/5 hover:text-white hover:bg-white/10'}`}>
        ...
        {activeTab === "Estatisticas" && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/[0.04] backdrop-blur-md border border-white/5 p-6 rounded-2xl flex flex-col items-center justify-center shadow-lg"><span className="text-3xl font-black text-white mb-1">{!dataLoaded ? <Loader2 className="w-5 h-5 animate-spin"/> : Object.keys(libraryData).length}</span><span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Salvos</span></div>
            ...
            <div className="bg-indigo-900/10 backdrop-blur-md border border-indigo-500/20 p-6 rounded-2xl flex flex-col items-center justify-center shadow-lg"><span className="text-3xl font-black text-indigo-400 mb-1">{!dataLoaded ? <Loader2 className="w-5 h-5 animate-spin"/> : obrasLidasIds.length}</span><span className="text-[10px] text-indigo-500/60 uppercase font-bold tracking-widest">Iniciadas</span></div>
            <div className="bg-red-900/10 backdrop-blur-md border border-red-500/20 p-6 rounded-2xl flex flex-col items-center justify-center shadow-lg"><span className="text-3xl font-black text-red-400 mb-1">{!dataLoaded ? <Loader2 className="w-5 h-5 animate-spin"/> : Object.values(libraryData).filter(s=>s==='Favoritos').length}</span><span className="text-[10px] text-red-500/60 uppercase font-bold tracking-widest">Favoritos</span></div>
          </div>
        )}

        {activeTab === "Historico" && (
            <div className="bg-white/[0.04] backdrop-blur-xl p-6 rounded-2xl border border-white/5 shadow-lg">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {libraryMangas.map(mg => (
                            <div key={mg.id} onClick={() => onNavigate('details', mg)} className="cursor-pointer group">
                                <div className="aspect-[2/3] w-full rounded-xl overflow-hidden relative border border-white/5 group-hover:border-cyan-500/30 transition-colors shadow-md">
        ...
        {activeTab === "Configuracoes" && (
            <div className="space-y-4">
                <div className="bg-white/[0.04] backdrop-blur-xl border border-white/5 rounded-2xl p-6 md:p-8 shadow-lg">
                ...
                <button onClick={() => setConfirmAction('cache')} className="flex items-center justify-between w-full bg-white/[0.04] backdrop-blur-xl border border-white/5 rounded-2xl p-6 md:p-8 hover:bg-white/10 transition-colors shadow-lg group">
