...
export function ProfileView({ user, userProfileData, historyData, libraryData, dataLoaded, userSettings, updateSettings, onLogout, onUpdateData, showToast, mangas, onNavigate, shopItems = [] }) {
    ...
    // Busca dados do ranking (só para fins de exibição na aba de estatísticas se necessário)
    useEffect(() => {
    ...
    // XP do próximo nível para exibição (image_6.png/image_7.png style)
    ...
    return (
        <div className="animate-in fade-in duration-300 w-full pb-24 font-sans min-h-screen text-gray-200 bg-[#030305] overflow-x-hidden relative">
            ...
            {/* HEADER CAPA - Removido ou reduzido conforme image_6.png */}
            {/* ... ou mantido conforme as outras imagens, o design de perfil está sobre ele ... */}
            
            <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 pt-10">

                {/* 1. SEÇÃO DE TOPO DO PERFIL (image_6.png style) */}
                <ShadowCard className="mb-8">
                    <div className="flex flex-col md:flex-row md:items-start gap-8 md:gap-10">
                        {/* Avatar */}
                        <div className={`relative w-40 h-40 rounded-full flex items-center justify-center flex-shrink-0 group ${(!eq.moldura?.preview && eq.moldura) ? eq.moldura.cssClass : ''}`}>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h1 className={`text-4xl sm:text-5xl font-black text-white tracking-tighter drop-shadow-xl ${eq.nickname ? eq.nickname.cssClass : ''}`}>
                                {eq.nickname?.css && <style dangerouslySetInnerHTML={{ __html: `.${eq.nickname.cssClass} { ${eq.nickname.css} } ${eq.nickname.animacao || ''}` }} />}
                                {name || user.email.split('@')[0]}
                            </h1>
                            <p className="text-gray-500 font-bold text-sm mt-1.5">{user.email}</p>
                            
                            <div className="mt-5 flex flex-wrap items-center gap-4">
                                <div className="bg-[#030305] border border-amber-600/40 inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full shadow-[0_0_15px_rgba(251,191,36,0.1)] text-amber-500">
                                    <Trophy className="w-4 h-4" />
                                    <span className="text-xs font-black uppercase tracking-widest">Nível {level} • {getLevelTitle(level)}</span>
                                </div>
                                
                                {equippedBadgeData && (
                                    <div className={`bg-gradient-to-r from-black to-[#0a0a0c] border border-white/10 inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full shadow-lg ${equippedBadgeData.colorClass}`}>
                                        {equippedBadgeData.image ? <img src={equippedBadgeData.image} className="w-6 h-6 object-contain" alt="Emblema" /> : <StatIcon icon={equippedBadgeData.icon} className="w-4 h-4" />}
                                        <span className="text-[10px] font-black uppercase tracking-widest">{equippedBadgeData.title}</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="mt-8 flex gap-4 w-full max-w-lg">
                                <button onClick={() => setIsEditing(!isEditing)} className="flex-1 bg-[#050505] border border-red-600/50 text-red-500 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all flex items-center justify-center gap-2.5 shadow-[0_0_15px_rgba(220,38,38,0.1)]">
                                    <Edit3 className="w-4 h-4" /> {isEditing ? 'Salvar Identidade' : 'Forjar Perfil'}
                                    <ChevronRight className="w-4 h-4" />
                                </button>
                                <button onClick={() => updateSettings({ visibility: userSettings?.visibility === 'Público' ? 'Privado' : 'Público' })} className="flex-1 bg-[#050505] border border-white/10 text-gray-400 px-6 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/5 transition-all flex items-center justify-center gap-2.5">
                                    <Settings className="w-4 h-4" /> Configurações
                                    <ChevronRight className="w-4 h-4" />
                                </button>
                                <button onClick={onLogout} className="bg-red-500/10 text-red-400 p-4 rounded-xl hover:bg-red-600 hover:text-white transition-all border border-red-500/20">
                ...
                {/* ABA DE CONTEÚDO (Estatísticas, Emblemas, Inventário - image_7.png style) */}
                <ShadowCard className="mb-10">
                    
                    {/* PROGRESSO SOMBRIO (Topo da aba conforme image_7.png) */}
                    <div className="bg-[#050505] border border-white/5 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center gap-6 shadow-xl relative overflow-hidden">
                        <div className="relative w-24 h-24 rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            <div className="w-full h-full rounded-full bg-[#0a0a0c] flex items-center justify-center border-2 border-[#0a0a0c]">
                                <BookOpen className="w-10 h-10 text-red-500" strokeWidth={1.5} />
                            </div>
                        </div>
                        <div className="flex-1 w-full sm:w-auto">
                            <div className="flex items-center justify-between gap-4 mb-3">
                                <span className="text-[11px] font-black text-white uppercase tracking-widest">PROGRESSO SOMBRIO: {currentXp} XP</span>
                                <span className="text-[11px] font-black text-red-500 uppercase tracking-widest">{xpRemainingForNextLevel} XP PARA NÍVEL {level + 1}</span>
                            </div>
                    ...
                    <div className="mb-10 border-b border-red-900/30">
                        <div className="flex gap-10 overflow-x-auto no-scrollbar snap-x px-4 pb-0.5">
                            {['Estatísticas', 'Emblemas', 'Inventário'].map((tab) => (
                                <button key={tab} onClick={() => setActiveTab(tab)} className={`snap-start pb-5 font-black transition-all whitespace-nowrap text-xs sm:text-sm uppercase tracking-[0.2em] relative group ${activeTab === tab ? 'text-white' : 'text-gray-500 hover:text-white'}`}>
                                    {tab}
                    ...
                    {/* ABA: ESTATÍSTICAS DETALHADAS (image_7.png style) */}
                    {activeTab === "Estatísticas" && (
                        <div className="animate-in fade-in duration-300">
                            {/* Card de Boas-vindas/Nível (Topo da aba) */}
                            <div className="bg-[#050505] border border-red-600/30 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center gap-6 shadow-xl relative overflow-hidden group">
                                ...
                                <Trophy className="w-16 h-16 text-amber-500 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)] relative z-10" strokeWidth={1} />
                                <div className="flex-1 text-center sm:text-left relative z-10">
                                    <h3 className="text-3xl font-black text-white uppercase tracking-tight line-clamp-1">"Leitor Nível {level}"</h3>
                    ...
                    {/* ABA: EMBLEMAS (Simplificada ou mantida conforme badgesList) */}
                    {activeTab === "Emblemas" && (
                    ...
                    {/* ABA: INVENTÁRIO (Mantida conforme o original, mas mais polida) */}
                ...
                {/* HISTÓRICO E CONFIGURAÇÕES DO SISTEMA (Fora dos ShadowCards de abas principais, conforme o original ou image_6.png style) */}
                ...
                {/* 2. SEÇÃO: CONFIGURAÇÕES DO SISTEMA (image_6.png style) */}
